  ...
  return (
    <>
      <nav className="bg-[#f5f2e8] border-b-2 border-[#2D2A26] px-8 py-4 flex items-center justify-between fixed top-0 left-0 right-0 w-full z-[9999] shadow-md">
            ...
            {(["staff", "tasks", "messaging"] as const).map((t) => (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2 rounded-md font-bold text-[11px] uppercase tracking-wider transition-all ${
                  activeTab === t ? "bg-[#2D2A26] text-white shadow-brutal -translate-y-0.5" : "text-gray-500 hover:bg-white/50"
                }`}
              >
        ...
        <div className="flex items-center gap-6">
          ...
          <Link href="/profile" className="w-9 h-9 bg-[#f5f2e8] border-2 border-[#2D2A26] flex items-center justify-center shadow-brutal hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5 transition-all overflow-hidden" title={userProfile?.full_name || "Profile"}>
            {userProfile && (
              <img src={userProfile.avatar_url || `https://api.dicebear.com/7.x/pixel-art/svg?seed=${userProfile.id}`} alt="Profile" className="w-full h-full object-cover p-0.5" />
            )}
          </Link>
      ...
      <div className="min-h-screen cork-texture flex flex-col font-sans text-[#2D2A26] pt-[75px]">
        ...
        {isCalendarOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[10000] transition-opacity" onClick={() => setIsCalendarOpen(false)} />
        )}
        ...
        {taskHeatmapOpenId && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[10000] transition-opacity" onClick={() => setTaskHeatmapOpenId(null)} />
        )}
