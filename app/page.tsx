...
function DashboardContent() {
  ...
  const updateTaskStatus = async (
    ...
    setBoardTasks((prev) => ({
      ...prev,
      [boardId]: { ...prev[boardId], status },
    }));
    setDrawings(
      drawings.map((d) => (d.id === boardId ? { ...d, completed } : d)),
    ); // Sync the top right checkbox!
    const { error } = await supabase
      .from("drawings")
  ...
  return (
    <>
      ...
      <div className="min-h-screen cork-texture flex flex-col font-sans text-[#2D2A26] pt-[75px]">
        ...
        <div
          className={`fixed top-0 right-0 h-full w-full max-w-3xl bg-[#f5f2e8] shadow-brutal-lg border-l-4 border-[#2D2A26] z-[10001] transform transition-transform duration-300 ease-in-out ${
                          ...
                          <div className="flex gap-2 shrink-0">
                            <button
                              ...
                              className={`flex-1 py-1.5 border-2 border-[#2D2A26] font-black text-[9px] uppercase transition-all shadow-sm ${
                                isPrimary
                                  ? "bg-[#2D2A26] text-white hover:bg-red-600 hover:border-red-600 hover:shadow-none translate-y-[1px]"
                            ...
                            <button
                              ...
                              className={`flex-1 py-1.5 border-2 border-[#2D2A26] font-black text-[9px] uppercase transition-all shadow-sm ${
                                isPrimary
                                  ...
                                  : isCollaborator
                                    ? "bg-[#86efac] text-gray-900 hover:bg-red-400 hover:shadow-none translate-y-[1px]"
                                    : "bg-white hover:bg-[#86efac]"
                              }`}
                            >
