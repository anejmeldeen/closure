...
function DashboardContent() {
  ...
  const updateBoardName = async (boardId: string, newName: string) => {
    setDrawings(
      drawings.map((d) => (d.id === boardId ? { ...d, name: newName } : d)),
    );
    setBoardTasks((prev) => ({
      ...prev,
      [boardId]: { ...prev[boardId], title: newName },
    }));
  ...
  const updateTaskDescription = async (
    ...
    setBoardTasks((prev) => ({
      ...prev,
      [boardId]: { ...prev[boardId], description },
    }));
  ...
  const updateTaskSkills = async (
    ...
    setBoardTasks((prev) => ({
      ...prev,
      [boardId]: { ...prev[boardId], required_skills },
    }));
    await supabase
      .from("drawings")
  ...
  const getTaskPersonnel = (boardId: string | null) => {
    ...
    if (task.collaborators)
      task.collaborators.forEach((id) => personnelIds.add(id));
    ...
    <>
      <nav className="bg-[#f5f2e8] border-b-2 border-[#2D2A26] px-8 py-4 flex items-center justify-between fixed top-0 left-0 right-0 w-full z-[9999] shadow-md">
        ...
        <div className="flex items-center gap-6">
          <Link
            href="/settings"
            className="hidden md:flex w-9 h-9 bg-gray-200 border-2 border-[#2D2A26] items-center justify-center text-[#2D2A26] shadow-brutal hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5 transition-all"
            title="Settings"
          >
            ...
            {userProfile && (
              <img
                src={
                  ...
                  `https://api.dicebear.com/7.x/pixel-art/svg?seed=${userProfile.id}`
                }
                alt="Profile"
          ...
          <button
            ...
            className="hidden md:block font-bold text-[10px] uppercase tracking-widest opacity-50 hover:opacity-100 transition-opacity"
          >
            Log Out
          </button>
          <button
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className="flex md:hidden w-9 h-9 border-2 border-[#2D2A26] items-center justify-center bg-white shadow-brutal-sm hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5 transition-all"
          >
            {mobileMenuOpen ? (
        ...
        <div className="fixed top-[65px] left-0 right-0 z-[9998] bg-[#f5f2e8] border-b-2 border-[#2D2A26] shadow-brutal md:hidden animate-in slide-in-from-top-2 duration-200">
          <div className="flex flex-col">
            {(["staff", "tasks", "messaging"] as const).map((t) => (
              <button
                ...
                className={`flex items-center gap-3 px-6 py-4 font-bold text-[11px] uppercase tracking-wider border-b border-[#2D2A26]/10 transition-all ${
                  activeTab === t
          ...
          <div className="flex items-center justify-between px-6 py-4">
            <Link
              ...
              className="flex items-center gap-2 font-bold text-[10px] uppercase tracking-widest opacity-60"
            >
      ...
      <div className="min-h-screen cork-texture flex flex-col font-sans text-[#2D2A26] pt-[75px]">
                      ...
                      {roomMembers.map((m) => (
                        <li key={m.id} className="flex items-center gap-3">
                          <Image
                            src={
                              m.avatar_url ||
                              `https://api.dicebear.com/7.x/pixel-art/svg?seed=${m.id}`
                            }
                            alt=""
                            width={24}
                            height={24}
                            unoptimized
                            className="rounded-full border border-[#2D2A26] bg-white object-cover aspect-square"
                          />
                  ...
                  {profiles.map((p) => (
                    ...
                    >
                      <Image
                        src={
                          p.avatar_url ||
                          `https://api.dicebear.com/7.x/pixel-art/svg?seed=${p.id}`
                        }
                        alt=""
                        width={64}
                        height={64}
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <div className="flex bg-[#2D2A26]/5 p-1 border-2 border-[#2D2A26]">
                      <button
                        ...
                        className={`px-4 py-1.5 font-black text-[10px] uppercase tracking-widest transition-all ${whiteboardSubTab === "todo" ? "bg-[#2D2A26] text-white shadow-brutal-sm" : "text-[#2D2A26] hover:bg-white/50"}`}
                      >
                        Active Tasks
                      </button>
                      <button
                        onClick={() => setWhiteboardSubTab("completed")}
                        className={`px-4 py-1.5 font-black text-[10px] uppercase tracking-widest transition-all ${whiteboardSubTab === "completed" ? "bg-[#2D2A26] text-white shadow-brutal-sm" : "text-[#2D2A26] hover:bg-white/50"}`}
                      >
                        Archive (7d)
                      </button>
                    </div>
                  ...
                  {filteredDrawings.map((draw) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          editingId !== draw.id &&
                          router.push(`/board/${draw.id}?tab=tasks`)
                        }
                        className={`group paper-texture min-h-[24rem] border-2 border-[#2D2A26] p-8 hover:translate-x-1.5 hover:translate-y-1.5 transition-all cursor-pointer flex flex-col justify-between relative animate-in zoom-in-95 fade-in duration-500 slide-in-from-top-2 ${draw.completed ? "shadow-brutal-lg hover:shadow-none bg-[#e5e7eb] opacity-80" : isInvolved ? "task-glow bg-[#f5f2e8]" : "shadow-brutal-lg hover:shadow-none bg-[#f5f2e8]"}`}
                      >
                        <div
                    ...
                    >
                      <Image
                        src={
                          ...
                          `https://api.dicebear.com/7.x/pixel-art/svg?seed=${msg.sender_id}`
                        }
                        alt=""
                        width={40}
                        height={40}
                        unoptimized
                        className="bg-white border-2 border-[#2D2A26] shadow-brutal-sm rounded-sm shrink-0 object-cover aspect-square"
                      />
                      <div
        ...
        <div
          className={`fixed top-0 right-0 h-full w-full max-w-4xl bg-[#f5f2e8] shadow-brutal-lg border-l-4 border-[#2D2A26] z-[10001] transform transition-transform duration-300 ease-in-out ${isCalendarOpen ? "translate-x-0" : "translate-x-full"}`}
        >
          <div className="h-full flex flex-col paper-texture">
            <div className="flex justify-between items-center p-8 border-b-4 border-[#2D2A26] bg-white">
              <h2 className="text-3xl font-black uppercase italic tracking-tighter flex items-center gap-3">
                <Calendar size={28} strokeWidth={3} /> Master Calendar
              </h2>
              <button
                onClick={() => setIsCalendarOpen(false)}
        ...
        {taskHeatmapOpenId && (
          <div
            className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[10000] transition-opacity"
            onClick={() => setTaskHeatmapOpenId(null)}
          />
        )}
        <div
          className={`fixed top-0 right-0 h-full w-full max-w-4xl bg-[#f5f2e8] shadow-brutal-lg border-l-4 border-[#2D2A26] z-[10001] transform transition-transform duration-300 ease-in-out ${taskHeatmapOpenId ? "translate-x-0" : "translate-x-full"}`}
        >
          <div className="h-full flex flex-col paper-texture">
            <div className="flex justify-between items-center p-8 border-b-4 border-[#2D2A26] bg-white">
              <div>
                <h2 className="text-3xl font-black uppercase italic tracking-tighter flex items-center gap-3 text-[#2D2A26]">
                  <Calendar size={28} strokeWidth={3} /> Task Availability
                </h2>
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-[#f5f2e8]">
              {taskHeatmapOpenId && (
                <TeamHeatmap
                  ...
                  subtitle="Showing schedules of users currently assigned to this task"
                />
              )}
            </div>
          </div>
            ...
            <div className="paper-texture bg-[#f5f2e8] border-4 border-[#2D2A26] p-8 max-w-4xl w-full shadow-brutal-lg max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-4 space-y-8">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <input
                        type="text"
                        value={
                          drawings.find((d) => d.id === detailsModalOpen)
                            ?.name || ""
                        }
                        onChange={(e) =>
                          updateBoardName(detailsModalOpen, e.target.value)
                        }
                        className="w-full p-3 border-2 border-[#2D2A26] font-black text-sm bg-white focus:outline-none"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] font-bold mb-1.5 uppercase opacity-60">
                      ...
                      <textarea
                        ...
                        className="w-full p-3 border-2 border-[#2D2A26] font-bold text-sm bg-white focus:outline-none resize-none leading-relaxed"
                      />
                    </div>
                  </div>
                  <div className="space-y-6">
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        ...
                        <select
                          value={
                            boardTasks[detailsModalOpen]?.priority || "Low"
                          }
                          onChange={(e) =>
                            updateTaskPriority(
                              detailsModalOpen,
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mb-3">
                        ...
                        <button
                          ...
                          className="px-4 py-2 bg-[#2D2A26] text-white font-black text-[10px] uppercase border-2 border-[#2D2A26] shadow-brutal-sm hover:translate-y-0.5 transition-all"
                        >
                          Add
                        </button>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {(
                          ...
                          <span
                            key={skill}
                            className="flex items-center gap-1 px-2.5 py-1 bg-[#bae6fd] border border-[#2D2A26] text-[9px] font-black uppercase shadow-sm"
                          >
                            {skill}
                            <button
                              onClick={() =>
                                updateTaskSkills(
                                  detailsModalOpen,
                                  (
                                    boardTasks[detailsModalOpen]
                                      ?.required_skills || []
                                  ).filter((s) => s !== skill),
                                )
                              }
                              className="ml-1 hover:text-red-600 font-black text-[12px] leading-none"
                            >
                              ×
                            </button>
                          </span>
                ...
                <div className="border-t-4 border-[#2D2A26] pt-6 mt-2">
                  <p className="text-[10px] font-bold mb-4 uppercase opacity-60 tracking-widest">
                  ...
                  <AutoAssignButton
                    ...
                    profiles={profiles}
                    onSuccess={(primaryId, collabs, newDescription) => {
                      setBoardTasks((prev) => ({
                        ...prev,
                        [detailsModalOpen]: {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {profiles.map((employee) => {
                      ...
                      return (
                        <div
                          key={employee.id}
                          className={`p-3 border-2 border-[#2D2A26] flex flex-col justify-between shadow-brutal-sm ${isPrimary ? "bg-[#ffbb00]/20" : "bg-white"}`}
                        >
                          <div className="flex items-center gap-3 min-w-0 mb-4">
                            <Image
                              src={
                                employee.avatar_url ||
                                `https://api.dicebear.com/7.x/pixel-art/svg?seed=${employee.id}`
                              }
                              alt=""
                              width={32}
                              height={32}
                              unoptimized
                              className="border-2 border-[#2D2A26] bg-white shrink-0 object-cover aspect-square"
                            />
                          ...
                          <div className="flex gap-2 shrink-0">
                            <button
                              ...
                              className={`flex-1 py-1.5 border-2 border-[#2D2A26] font-black text-[9px] uppercase transition-all shadow-sm ${isPrimary ? "bg-[#2D2A26] text-white hover:bg-red-600 hover:border-red-600 hover:shadow-none translate-y-[1px]" : "bg-white hover:bg-[#ffbb00]"}`}
                            >
                            ...
                            <button
                              ...
                              disabled={isPrimary}
                              className={`flex-1 py-1.5 border-2 border-[#2D2A26] font-black text-[9px] uppercase transition-all shadow-sm ${isPrimary ? "opacity-30 cursor-not-allowed bg-gray-200" : isCollaborator ? "bg-[#86efac] text-gray-900 hover:bg-red-400 hover:shadow-none translate-y-[1px]" : "bg-white hover:bg-[#86efac]"}`}
                            >
                ...
                {(["Critical", "High", "Medium", "Low"] as const).map(
                  (priority) => (
                    <button
                      key={priority}
                      onClick={() =>
                        updateTaskPriority(priorityEditId, priority)
                      }
