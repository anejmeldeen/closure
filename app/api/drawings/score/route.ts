...
export async function POST(request: Request) {
  try {
    ...
    const enrichedProfiles = allProfiles.map((profile: any) => {
      ...
      const utilization = (profile.task_hours_7d || 0) / (profile.max_capacity || 40);
      ...
      const candidates = enrichedProfiles
        .map(p => {
          const matchedSkills = drawingSkills.filter((s: string) => 
            (p.skills || []).some((ps: string) => ps.toLowerCase().includes(s.toLowerCase()))
          );
          ...
          const score = skillBonus + (p.performance_rating || 3) - batchPenalty - utilizationPenalty;
        ...
        if (parsed && Array.isArray(parsed.team)) {
          parsed.team.forEach((member: any) => {
            ...
            if (prof) {
              ...
              prof.utilization = ((prof.task_hours_7d || 0) + hours) / (prof.max_capacity || 40);
            }
          });
          finalProposals.push({ ...parsed, drawingId: drawing.id, drawingName: drawing.title || drawing.name });
        }
      } catch (err) {
