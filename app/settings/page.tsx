...
export default function SettingsPage() {
  ...
  const handleSync = async () => {
    ...
    try {
      const res = await fetch('/api/calendar/sync', {
        ...
        body: JSON.stringify({ profile_id: profile.id, week_start: new Date().toISOString().split('T')[0] }),
      });
  ...
  // Define tabs for the sidebar map
              ...
              <section className="paper-texture border-4 border-[#2D2A26] shadow-brutal-lg p-8">
                ...
                <div className="space-y-6">
                  <p className="text-xs font-bold uppercase opacity-60 leading-relaxed max-w-xl">
                    Sync your external schedule to automatically block off busy hours in the personnel registry. Your events remain private.
                  </p>
                  
                  <button 
                    onClick={handleSync}
                    disabled={syncing}
                    className="max-w-md w-full flex items-center justify-center gap-3 p-4 bg-white border-2 border-[#2D2A26] font-black text-xs uppercase shadow-brutal hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all disabled:opacity-50"
                  >
                    {syncing ? (
                      <RefreshCcw size={16} className="animate-spin text-red-600" />
                    ) : (
                      <div className="w-5 h-5 bg-red-100 text-red-600 rounded-full flex items-center justify-center font-bold text-[10px]">G</div>
                    )}
                    {syncing ? "Authenticating..." : "Connect Google Calendar"}
                  </button>
                </div>
              </section>
            </div>
