...
import { 
  ArrowLeft, Plus, X, Image as ImageIcon, Save, 
  Calendar, Bell, Mail, Smartphone, RefreshCcw, 
  User, Wrench, CheckCircle2 
} from "lucide-react";
...
export default function SettingsPage() {
  ...
  const handleGoogleDisconnect = async () => {
    if (!profile?.id) return;
    const confirmDisconnect = window.confirm("Are you sure you want to disconnect your Google Calendar? You will no longer be able to sync events.");
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      alert(`Success! Imported ${data.slotsAdded} busy blocks to your current week.`);
    } catch (err: any) {
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-10">
              <section className="paper-texture border-4 border-[#2D2A26] shadow-brutal-lg p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <p className="text-[10px] font-black uppercase mb-2">Full Name</p>
                ...
                <div className="space-y-8">
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input type="text" value={customUrl} onChange={(e) => setCustomUrl(e.target.value)} placeholder="https://example.com/my-image.jpg" className="flex-1 p-3 border-2 border-[#2D2A26] font-bold text-xs bg-white focus:outline-none shadow-brutal-sm" />
                      <button onClick={handleCustomUrlSave} className="px-6 bg-[#2D2A26] text-white font-black text-[10px] uppercase border-2 border-[#2D2A26] shadow-brutal-sm hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-5 md:grid-cols-10 gap-2">
                      {AVATAR_SEEDS.map((seed) => {
                        ...
                        return (
                          <button 
                            key={seed} onClick={() => updateAvatar(url)} 
                  ...
                  {(profile?.skills || []).length === 0 ? (
                    ...
                    (profile?.skills || []).map((skill) => (
                      <span key={skill} className="group flex items-center gap-1.5 px-3 py-1.5 bg-[#2D2A26] text-white text-[10px] font-black uppercase border-2 border-[#2D2A26] shadow-sm">
                        {skill}
                        <button onClick={() => removeSkill(skill)} className="opacity-40 hover:opacity-100 hover:text-red-400 transition-colors"><X size={12} strokeWidth={3} /></button>
                      </span>
                    ))
                  )}
                </div>
                <div className="flex gap-2">
                  <input value={newSkill} onChange={(e) => setNewSkill(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addSkill(); }} placeholder="E.g. React, Logistics, DevOps..." className="flex-1 border-2 border-[#2D2A26] px-4 py-3 text-[12px] font-bold uppercase bg-white outline-none shadow-brutal-sm" />
                  <button onClick={addSkill} className="px-6 bg-[#86efac] text-[#2D2A26] border-2 border-[#2D2A26] hover:bg-[#ffbb00] transition-all shadow-brutal-sm hover:shadow-none hover:translate-x-[1px] hover:translate-y-[1px] flex items-center gap-2 font-black text-[10px] uppercase"><Plus size={14} strokeWidth={3} /> Add</button>
                </div>
              </section>
            </div>
          )}
              ...
              <section className="paper-texture border-4 border-[#2D2A26] shadow-brutal-lg p-8">
                <div className="mb-6 pb-4 border-b-4 border-[#2D2A26]">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-2xl font-black uppercase italic tracking-tight">Alert Preferences</h2>
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 border-2 border-[#2D2A26] bg-white shadow-brutal-sm">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <span className="block font-black text-xs uppercase tracking-widest">Email Alerts</span>
                        <span className="text-[10px] font-bold opacity-50">Daily digests and mentions</span>
                      </div>
                    </div>
                    <button onClick={() => setEmailNotif(!emailNotif)} className={`w-12 h-6 border-2 border-[#2D2A26] relative transition-colors ${emailNotif ? 'bg-[#86efac]' : 'bg-gray-200'}`}>
                      <div className={`w-4 h-4 bg-[#2D2A26] absolute top-0.5 transition-all ${emailNotif ? 'left-6' : 'left-1'}`}></div>
                    </button>
                  </div>
                  <div className="flex items-center justify-between p-4 border-2 border-[#2D2A26] bg-white shadow-brutal-sm">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <span className="block font-black text-xs uppercase tracking-widest">In-App Alerts</span>
                        <span className="text-[10px] font-bold opacity-50">Instant push notifications inside Closure</span>
                      </div>
                    </div>
                    <button onClick={() => setAppNotif(!appNotif)} className={`w-12 h-6 border-2 border-[#2D2A26] relative transition-colors ${appNotif ? 'bg-[#86efac]' : 'bg-gray-200'}`}>
                      <div className={`w-4 h-4 bg-[#2D2A26] absolute top-0.5 transition-all ${appNotif ? 'left-6' : 'left-1'}`}></div>
                    </button>
                  </div>
                </div>
              </section>
              ...
              <section className="paper-texture border-4 border-[#2D2A26] shadow-brutal-lg p-8">
                <div className="mb-6 pb-4 border-b-4 border-[#2D2A26]">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-2xl font-black uppercase italic tracking-tight">Connections</h2>
                  </div>
                </div>

                <div className="space-y-6">
                  <p className="text-xs font-bold uppercase opacity-60 leading-relaxed max-w-xl">
                    Sync your external schedule to automatically block off busy hours in the personnel registry. Your events remain private, and you can manually edit the synced data later.
                  </p>
                  ...
                  {isGoogleLinked ? (
                    ...
                    >
                      <div className="w-5 h-5 bg-red-100 text-red-600 rounded-full flex items-center justify-center font-bold text-[10px]">G</div>
