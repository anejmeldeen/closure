...
import { RefreshCcw, CheckCircle2, X, AlertTriangle, Activity, User, Check } from "lucide-react";
...
export default function BulkAutoAssignButton({ tasks, profiles, onSuccess }: BulkAutoAssignProps) {
  ...
  const handleFetchBulk = async () => {
    ...
    try {
      const d = new Date();
      const monday = new Date(d.setDate(d.getDate() - d.getDay() + 1)).toISOString().split('T')[0];
      const { data: avail } = await supabase.from("availability_slots").select("*").eq("week_start_date", monday);

      const res = await fetch('/api/drawings/score', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ drawings: unassigned, allProfiles: profiles, allAvailability: avail || [] })
      });
  ...
  const handleApproveSelected = async () => {
    ...
    const approvedProposals = currentProposalsSnapshot.filter(p => selectedIds.has(p.drawingId));

    try {
      const updates = [];
      for (const prop of approvedProposals) {
        if (!prop.team?.length) continue;
        const sortedTeam = prop.team.sort((a: any, b: any) => b.allocated_hours - a.allocated_hours);
        ...
        await supabase.from("drawings").update({ assigned_to: primaryId, collaborators: collabs, description: desc }).eq("id", prop.drawingId);
        updates.push({ id: prop.drawingId, assigned_to: primaryId, collaborators: collabs, description: desc });
        
        for (const t of prop.team) {
          const { data: p } = await supabase.from("profiles").select("task_hours_7d").eq("id", t.id).single();
          await supabase.from("profiles").update({ task_hours_7d: (p?.task_hours_7d || 0) + t.allocated_hours }).eq("id", t.id);
        }
      }
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-xl paper-texture border-4 border-[#2D2A26] bg-white shadow-brutal-lg flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-300">
            ...
            <div className="flex items-center justify-between p-6 border-b-4 border-[#2D2A26] bg-white">
              ...
              <button onClick={() => setPendingProposals(null)} className="p-2 border-2 border-[#2D2A26] shadow-brutal-sm hover:translate-y-0.5 transition-all bg-white">
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-start mb-3 border-b-2 border-[#2D2A26]/10 pb-2">
                          <p className="font-black text-xs uppercase">{p.drawingName}</p>
                          <div className={`w-5 h-5 border-2 border-[#2D2A26] flex items-center justify-center ${isSelected ? 'bg-[#2D2A26] text-white' : 'bg-white'}`}>
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                          {p.team?.map((member: any) => (
                            <span key={member.id} className="bg-[#2D2A26] text-white px-2 py-1 text-[9px] font-black uppercase flex items-center gap-1.5">
                              <User size={10} /> {member.name} <span className="opacity-40">|</span> {member.allocated_hours}H
                            </span>
                          ))}
                        </div>
            ...
            {pendingProposals && (
              <div className="p-6 border-t-4 border-[#2D2A26] bg-white flex gap-4 shrink-0">
                ...
                >
                  {isApplying ? <RefreshCcw size={16} className="animate-spin" /> : <CheckCircle2 size={16} />}
                  {isApplying ? "Committing..." : `Initialize ${selectedIds.size} Changes`}
                </button>
              </div>
            )}
          </div>
