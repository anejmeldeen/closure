...
export default function TeamHeatmap({
  ...
  const heatMapData = useMemo(() => {
    ...
    profiles.forEach((profile) => {
      ...
      const currentWeekRecord = availabilityRecords.find(
        (r: any) => r.week_start_date === weekKey,
      );
  ...
  const getHeatColor = (busyCount: number) => {
    ...
    if (ratio <= 0.5) return "bg-[#ffbb00] border-[#2D2A26]";
  ...
  return (
    <div className="paper-texture bg-white border-2 border-[#2D2A26] p-4 sm:p-8 shadow-brutal flex flex-col h-full min-h-[600px]">
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between mb-6 pb-4 border-b-2 border-[#2D2A26] gap-4">
        <div>
          ...
          <p className="text-[10px] font-bold opacity-50 uppercase mt-1">
            {subtitle ||
              `Displaying ${profiles.length} users overlapping schedules`}
          </p>
        </div>
          ...
          {HOURS.map((hour) => (
            <div key={hour} className="grid grid-cols-8">
              ...
              {Array.from({ length: 7 }).map((_, i) => {
                ...
                const cellData = heatMapData[slotKey] || {
                  count: 0,
                  names: [],
                };
                ...
                return (
                  <div
                    key={slotKey}
                    className={`border-r-2 border-b-2 border-[#2D2A26] h-10 flex items-center justify-center transition-colors relative group ${colorClass}`}
                  >
                    {isBusy && (
                      <>
                        ...
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:flex flex-col bg-[#2D2A26] text-white text-[9px] font-bold p-3 z-[200] w-40 shadow-brutal pointer-events-none animate-in fade-in zoom-in-95 duration-200">
                          ...
                          <ul className="space-y-1">
                            {cellData.names.map((name: string, idx: number) => (
                              <li
                                key={idx}
                                className="truncate uppercase tracking-tight text-white/90"
                              >
                                • {name}
                              </li>
                            ))}
      ...
      <div className="mt-8 flex items-center justify-between border-t-2 border-[#2D2A26] pt-4 shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-1.5">
            <div className="w-4 h-4 bg-white border-2 border-[#2D2A26]" />
            <span className="text-[9px] font-black uppercase">
              Full Team Free
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-4 h-4 bg-[#fef08a] border-2 border-[#2D2A26]" />
            <span className="text-[9px] font-black uppercase">Light</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-4 h-4 bg-[#ffbb00] border-2 border-[#2D2A26]" />
            <span className="text-[9px] font-black uppercase">Moderate</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-4 h-4 bg-[#f97316] border-2 border-[#2D2A26]" />
            <span className="text-[9px] font-black uppercase">Heavy</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-4 h-4 bg-[#dc2626] border-2 border-[#2D2A26]" />
            <span className="text-[9px] font-black uppercase">
              Full Team Busy
            </span>
          </div>
